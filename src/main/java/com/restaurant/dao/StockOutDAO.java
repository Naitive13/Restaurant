...
public class StockOutDAO implements CrudDAO<Stock> {
  private final Datasource datasource;

  public StockOutDAO(Datasource datasource) {
  ...
  public List<Stock> get(List<Criteria> criteriaList, int pageIndex, int pageSize) {
    ...
    String query =
        "SELECT stock_out.ingredient_id, stock_out.quantity, "
            + "ingredient.unit, stock_out.last_modified FROM stock_out "
            + "JOIN ingredient ON ingredient.ingredient_id=stock_out.ingredient_id WHERE 1=1 ";
      ...
      if (criteria.getClass() == Filter.class) {
        switch (criteria.getColumn()) {
          ...
          case "quantity" -> {
            ...
            query = query + " AND stock_out.quantity=? ";
            quantityIndex = counter;
            counter++;
          }
          case "ingredient_id" -> {
            ingredientId = (long) ((Filter) criteria).getValue();
            query = query + " AND stock_out.ingredient_id=? ";
          ...
          case "last_modified" -> {
            ...
            query = query + " AND stock_out.last_modified BETWEEN ? and ? ";
  ...
  public double getTotalForIngredient(long ingredientId) {
    String query =
        "SELECT ingredient_id, sum(quantity) as total "
            + "FROM stock_out GROUP BY ingredient_id "
